...
## 23. V1.6 基础层：描述统计与分布可视化
### 23.1 目标
落地数据分析方法论"一、描述性统计分析（基础层）"，补齐：扩展统计量（CV/偏度/峰度/分位数/IQR/缺失率）、分布可视化（直方图+KDE、箱线图、Q-Q 图）、相关性（相关矩阵热力图）、散点图矩阵。
### 23.2 架构改动
- UI 引入"分析模式"概念：顶部三个可切换按钮（折线趋势 / 描述统计 / 时序监控），左侧参数面板用 QStackedWidget 切换，右侧图表区用 QTabWidget 承载不同分析视图；下方结果区由单一 StatsPanel 改为 MultiTablePanel 多表切换。
- 新增服务：app/services/descriptive_service.py
  - `StatsResult` 扩展字段：missing_rate/cv/skewness/kurtosis/q1/q3/iqr/p01/p05/p95/p99
  - `batch_descriptive_stats / descriptive_to_dataframe` 扩展综合统计
  - `quantile_table / missing_summary / correlation_matrix / boxplot_stats`
  - `distribution_data` 输出直方图分箱、Silverman KDE（纯 numpy 实现，不引 scipy）、IQR 离群点
- 新增 UI：
  - app/ui/widgets/descriptive_panel.py（描述统计配置面板）
  - app/ui/widgets/histogram_chart.py / boxplot_chart.py / qq_chart.py / correlation_chart.py / scatter_matrix_chart.py
  - app/ui/widgets/descriptive_charts_panel.py（多 Tab 图容器）
  - app/ui/widgets/multi_table_panel.py（多结果表切换）
- MainWindow：
  - 所有重计算走现有 Worker/QThreadPool 后台执行，不阻塞 UI
  - `_run_descriptive_analysis` 完成"计算→表格渲染→图表渲染→切换到结果区"
  - 导出适配："导出统计结果"随当前选中表导出；"导出图表"随当前可见图表 Tab 导出
### 23.3 注意点
- KDE 实现为 Silverman 带宽 + 高斯核，纯 numpy 向量化，不引入 scipy；Q-Q 理论分位数使用 Acklam 近似算法（精度 ~1e-9）。
- 散点矩阵默认每格最多 2000 点，列数超过 6 列自动截断，避免 N*N 爆炸。
- 描述统计对非数值列会在"缺失/无效"表中标注，但直方图/箱线/Q-Q 仅对数值列绘制。
- 折线趋势模式功能与 V1.5 完全等价，原有时间粒度聚合/降采样/tooltip/时区语义不变。
...
### ????
- ?? `app/ui/widgets/boxplot_chart.py` ?????????????
  - ??`pg.mkPen("#b00", 1.2)`
  - ??`pg.mkPen(QColor("#b00"), width=1.2)`
  - ???pyqtgraph `mkPen(color, width)` ????????????? intColor ???? "#b00" ?? index ???? `ValueError: invalid literal for int() with base 10: '#b00'`?
- ???? 5 ??? `clear()` ????????
  - histogram/boxplot/qq/correlation ?? `pg.TextItem` ????"?????? ???????? ?? xxx"
  - scatter_matrix ?? `pg.LabelItem` ??"?????? ????? ??? ????????"
  - ?????`app/ui/widgets/{histogram_chart,boxplot_chart,qq_chart,correlation_chart,scatter_matrix_chart}.py`
- ?????? `MainWindow._prepare_chart_data` ???
  - ?? `MainWindow._dedup_columns(cols)` ???????????? `__dupN` ???
  - ? `y_series` ????????????????
  - ? `df.columns.duplicated()` ????????????? `df[c]` ?? DataFrame ?? `pd.to_numeric` ? `TypeError: arg must be a list, tuple, 1-d array, or Series`?

### ?????
- ???? `tests/test_descriptive_service.py`?9 ???????
- ????????? `tests/run_functional_tests.py`?
  - ???`tests/D3_7#_B1_85_0.csv`?13575 ? ? 6 ??
  - ???file_loader ????stats_service/descriptive_service ?????time_aggregation 6 ??? + ?????data_processing ?????export_service CSV/XLSX ?????infer_numeric/infer_datetime?
  - ??????80 ?????
- ?? UI ???? `tests/ui_smoke_test.py`?offscreen QPA??
  - ????? ? ?????hist/box/qq/corr ???CSV/XLSX ???? ?????/??/????????+PNG ???? ?????delete_row ??????? ???????? ? ???
  - ????? `tests/ui_smoke_out/`?
- ??????????20260702 ?? 6 ? CSV??? 80918 ???merge?processing???/?????????

### ????
```powershell
.\.venv\Scripts\python.exe tests\test_descriptive_service.py
.\.venv\Scripts\python.exe tests\run_functional_tests.py
.\.venv\Scripts\python.exe tests\ui_smoke_test.py
```

### ????
- ?????`logs/app_YYYY-MM-DD.log`??????
- UI ???"??????"?????? logs/ ??
- Worker ????? AppLogger ? `[ERROR]` ???? traceback

---

## 24. V1.7 跨类合同图开发记录（2026-07-10/11）

### 24.1 目标
落地 ADR-002（见 `../../docs/adr/ADR-002-v1.7-cross-category.md`）：引入机头 / 机尾两类文件概念，两类独立单像素精度 factor；同类别内部沿用 concat+时间排序，跨类别按 `时间` 做 outer join 得到合同图数据集；同一张折线图支持同时勾选 [机头]/[机尾] 前缀列进行多 Y 对比。

### 24.2 模块改动点
- `app/models/dataset_item.py`：`DatasetItem` 新增 `category`（`head`/`tail`/`None`）、`pixel_factor`、`scaled` 三个字段，默认值保证旧构造/旧 pickle 向后兼容。
- `app/services/data_processing.py`：新增 `scale_datasets_by_category(manager, category, factor, exclude_mode, exclude_columns)`，仅作用于 `kind=='original'` 且类别匹配的原始数据集，`scaled=True` 的集合自动跳过防重复乘；沿用 V1.6.1 float32 单精度、排除列、自动跳时间/文本/布尔/日期、`(mm)` 后缀、重名去重、非法因子校验语义。
- `app/services/dataset_manager.py`：新增类别常量 `CATEGORY_LABELS`/`CATEGORY_PREFIXES`；新增 `merge_by_category(category)`（同类别 concat+时间排序，结果命名 `机头_合并`/`机尾_合并`，已存在则复用）、`merge_cross_category()`（`时间` outer join，非时间列加 `[机头]`/`[机尾]` 前缀，已有 `(mm)` 后缀保持在末尾，单侧缺失保持 NaN）、`merge_uncategorized()`（未分类旧路径 0 回归入口）。
- `app/ui/main_window.py`：
  - 工具栏新增“导入机头文件”/“导入机尾文件”按钮，通过 `QInputDialog.getDouble` 弹出 factor 对话框，QSettings 记忆 `head_pixel_factor`/`tail_pixel_factor`；
  - 导入完成回调 `_on_category_import_done` 为 `DatasetItem` 打 category/factor/scaled 标记；
  - 新增 `_merge_by_category` / `_merge_cross_category` / `_scale_category_datasets` 槽；
  - 关于对话框版本说明升级至 V1.7.0；
  - 跨类合并数据集激活后，列选择/时间粒度/描述统计全部复用现有通道，无需额外分支。
- `app/ui/widgets/dataset_panel.py`：数据集列表项增加类别徽标（机头蓝/机尾橙）和“机头合并/机尾合并/跨类合同图”三个操作按钮，新增 `merge_head_requested`/`merge_tail_requested`/`merge_cross_requested` 信号。
- `app/ui/widgets/processing_panel.py`：“作用范围”下拉新增“所有机头/所有机尾”，`scale_category_requested` 信号携带类别/factor/排除信息；`set_category_factor` 用于 UI 同步记忆的 factor。
- `app/ui/widgets/chart_config_panel.py` / `column_panel.py` / `chart_panel.py`：无需结构性改动，带 `[机头]`/`[机尾]` 前缀的列名直接复用多Y选择/图例/tooltip/时间粒度聚合通道；时间粒度 outer join 聚合对各列分别 mean，缺失窗口自然为 NaN，折线断开。

### 24.3 测试
- `tests/test_cross_category.py`（新增，W1）：覆盖同类别合并（concat+排序）、跨类合并 outer join 形状、非时间列加前缀、`(mm)` 后缀保留在末尾、时间重叠/先后情形下单侧 NaN、未分类旧路径回归。
- `tests/test_scale_feature.py`（扩展，W1）：补“按类别批量缩放”用例，验证只作用于指定类别、已缩放集合跳过、列名 `(mm)` 后缀出现、非法 factor/1.0 行为、float32 路径。
- `tests/test_descriptive_service.py`：保持 W1 修过的相对 sys.path，保持基线全绿。
- `tests/ui_smoke_test.py`（扩展，W3）：保留原单类路径（导入→描述统计→折线→3 种粒度→数据处理→处理后描述→清空），新增跨类路径：编程式构造机头/机尾小 DataFrame → `DatasetManager.import_file` 后直接设置 `category/pixel_factor/scaled`（绕过文件对话框）→ 依次调用 `merge_by_category('head')`/`merge_by_category('tail')`、`merge_cross_category()` → 激活跨类合并数据集 → 默认选 `时间` 为 X，勾选至少一个 `[机头]` 列 + 一个 `[机尾]` 列，遍历 6 种时间粒度（原始/分钟/小时/班次/天/周）触发 `_run_chart_only()` 不抛异常 → 切回描述统计运行一次 → 调用 `scale_datasets_by_category('head', factor, ...)` 验证 `(mm)` 后缀出现且列数不变 → 清空。脚本顶部 `os.environ["QT_QPA_PLATFORM"]="offscreen"` 在 import PySide6 之前设置。

### 24.4 硬约束与回归
- 所有改动文件 UTF-8 无 BOM、LF 换行。
- 未引入新第三方依赖。
- 旧“导入文件/导入文件夹”（category=None 未分类路径）保持 V1.6.1 行为 0 回归；`DatasetManager.merge_uncategorized()` 专门保留该入口。
- compileall 0 报错；`test_scale_feature` / `test_cross_category` / `test_descriptive_service` 全绿；`ui_smoke_test.py` offscreen 单类+跨类路径无崩溃。

### 24.5 未覆盖 / 遗留（V1.7 范围外）
- 跨类合并结果的端到端导出（PNG/CSV/XLSX）人工走查；本版本只保证服务层和 UI 通道连通。
- “导入文件夹”批量导入机头/机尾的类别+factor 流程（当前文件夹导入走未分类路径）。
- factor 修改后对已缩放/已合并数据集给出“需要重缩放/重合并”的提示与一键刷新按钮（当前需要用户手动重新执行缩放 / 删除合并数据集后重新合同图）。
- 第三类/多类别扩展、类别自定义命名、时间容差/最近邻对齐、缺失值填充策略均不在 V1.7 范围。
